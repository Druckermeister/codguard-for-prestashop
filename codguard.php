...
class CodGuard extends Module
{
    ...
    public function hookDisplayPaymentTop($params)
    {
        ...
        // We don't show the banner here because displayPaymentTop fires too early (address step)
        // The JavaScript will handle showing the message on the payment selection step
        if (isset($this->context->cookie->codguard_blocked) && $this->context->cookie->codguard_blocked == '1') {
            PrestaShopLogger::addLog('CodGuard [displayPaymentTop]: Customer is blocked (cookie flag set)', 1);
            // Config will be read by JavaScript from the data attribute or cookie
        }
        ...
        if ($rating_percentage < $tolerance) {
            ...
            $html .= '<div data-codguard-config=\''.json_encode($config).'\' style="display:none;"></div>';
            $html .= '<!-- CodGuard: Config element added -->';

            // Note: We do NOT add the warning banner here because displayPaymentTop fires too early
            // The banner will be shown by codguard.js on the payment selection step
